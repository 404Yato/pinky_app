...
export function BookDetailPage({ bookId, onBack, onEdit, onDeleted, successMessage }) {
  ...
  return (
    <div>
      ...
      <article className="grid gap-8 md:grid-cols-[minmax(14rem,20rem)_minmax(0,1fr)] lg:gap-12">
        <div className="mx-auto w-full max-w-80 overflow-hidden rounded-lg border border-border bg-secondary shadow-[0_10px_30px_rgb(59_42_32_/_0.12)] md:mx-0">
          <BookCover book={book} />
        </div>
        ...
        <div className="min-w-0 py-1">
          ...
          <h1 className="mt-5 font-heading text-4xl font-semibold leading-tight tracking-tight text-foreground sm:text-5xl">{book.title}</h1>
          ...
          <div className="mt-7 flex flex-wrap gap-3 border-y border-border py-5">
            <Button type="button" onClick={onEdit} variant="outline" className="h-11 gap-2 px-4 text-sm"><PencilSimple aria-hidden="true" /> Editar</Button>
            <Button type="button" onClick={handleFavorite} disabled={updating} variant={book.favorite ? "secondary" : "outline"} className="h-11 gap-2 px-4 text-sm"><Heart aria-hidden="true" weight={book.favorite ? "fill" : "regular"} /> {book.favorite ? "Quitar favorito" : "Marcar favorito"}</Button>
            <Button type="button" onClick={() => setDeleteOpen(true)} variant="destructive" className="h-11 gap-2 px-4 text-sm"><Trash aria-hidden="true" /> Eliminar</Button>
          </div>
            ...
            <dl className="mt-4 grid gap-x-8 gap-y-5 rounded-lg border border-border bg-card p-5 sm:grid-cols-2 sm:p-6">
              {metadata.map(([label, value]) => (
                <div key={label}>
                  ...
                  <dd className="mt-1 text-sm font-medium text-foreground">{value || "No disponible"}</dd>
                </div>
              ))}
            </dl>
          </section>
