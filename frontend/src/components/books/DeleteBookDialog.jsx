...
export function DeleteBookDialog({ book, open, onClose, onDeleted }) {
  ...
  return (
    <dialog ref={dialogRef} aria-labelledby="delete-book-title" aria-describedby="delete-book-description" aria-busy={deleting} onCancel={handleCancel} onClose={onClose} className="modal-dialog m-auto max-h-[calc(100dvh-2rem)] w-[min(28rem,calc(100%-2rem))] max-w-none overflow-y-auto rounded-lg border border-border bg-card p-0 text-foreground shadow-2xl">
      <div className="p-5 sm:p-7">
        ...
        <div className="mt-7 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
          <Button ref={cancelButtonRef} type="button" variant="outline" onClick={onClose} disabled={deleting} className="h-11 px-5 text-sm">Conservar libro</Button>
