...
export function LibraryEmptyState({ filtered, onReset, title, description, Icon: EmptyIcon }) {
  ...
  return (
    <section className="rounded-lg border border-dashed border-border bg-card/60 px-6 py-16 text-center" aria-labelledby="empty-library-title">
      ...
      <p className="mx-auto mt-2 max-w-md text-sm leading-6 text-muted-foreground">
        {filtered ? "Prueba con otro título, autor o una combinación diferente de filtros." : (description ?? "Agrega un libro para comenzar.")}
      </p>
